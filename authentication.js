...
const getFields = (z, bundle) => {
  var includeTesting = false;
  if(process.env.INCLUDE_TESTING_ENVIRONMENT != null && process.env.INCLUDE_TESTING_ENVIRONMENT.toLowerCase() === 'true')
    includeTesting = true;
  const testingLabels = [
      // They are for testing only:
      { label: 'Senseforce testing environment', sample: 'https://test.senseforce.io', value: 'https://test.senseforce.io'}];
  
  const productionLabels = [
      { label: 'Senseforce production environment', sample: 'https://production.senseforce.io', value: 'https://production.senseforce.io'}];
  
  const allLabels = (includeTesting ? testingLabels : []).concat(productionLabels);
  return [
      { key: 'domain', type: 'string', choices: allLabels, required: true },
      { key: 'env variables', type: 'string', choices: process.env, required: true }
    ];
  }
